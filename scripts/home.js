...
const loadOpenCards = () => {
    fetch("https://phi-lab-server.vercel.app/api/v1/lab/issues") // Promise of Response
        .then(res => res.json()) // Promise of json data
        .then(json => displayOpenCard(json.data));
};
...
const displayOpenCard = (cards) => {
    ...
    // 1. FILTER FIRST (This prevents empty gaps)
...
const loadClosedCards = () => {
    fetch("https://phi-lab-server.vercel.app/api/v1/lab/issues") // Promise of Response
        .then(res => res.json()) // Promise of json data
        .then(json => displayClosedCard(json.data));
};

const displayClosedCard = (cards) => {
    // 1. Get the container & empty
    const cardContainer = document.getElementById("closed-container");
    cardContainer.innerHTML = "";
